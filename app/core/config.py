...
class Settings(BaseModel):
    ...
    # Search defaults & emulation
    DEFAULT_QUERY: str = "software engineer"
    DEFAULT_LOCATION: str = "Bangladesh"
    TIMEZONE: str = os.environ.get("TIMEZONE", "Asia/Dhaka")
    LOCALE: str = os.environ.get("LOCALE", "en-US")

    # Optional Proxy Configuration (for Render / cloud deployments to avoid 429 rate limits)
    PROXY_SERVER: Optional[str] = os.environ.get("PROXY_SERVER") or os.environ.get("HTTP_PROXY") or os.environ.get("HTTPS_PROXY")
    PROXY_USERNAME: Optional[str] = os.environ.get("PROXY_USERNAME")
    PROXY_PASSWORD: Optional[str] = os.environ.get("PROXY_PASSWORD")

    def get_li_at_cookie(self) -> str:
        """
        Retrieves the li_at cookie with priority:
        1. LI_AT environment variable
